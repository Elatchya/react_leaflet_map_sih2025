...
// Mapping for lane to UI movements
...
// Icon chooser based on congestion
...
// AlertBox component for high congestion alerts
...
// MapPage with marker click navigation
function MapPage() {
  const navigate = useNavigate();
  ...
  return (
    <div className="map-page">
      <Header />
      <MapContainer
        center={[junctionData.lat, junctionData.lng]}
        zoom={17}
  ...
  eventHandlers={{
    click: () => {
      window.location.href = '/junction_details.html'; // Directly open static file
    }
  }}
>
          <Tooltip>
...
// SignalCard to display signal details per direction
function SignalCard({ direction, junctionData }) {
  const getLaneData = (dir, mov) => {
    const laneId = Object.keys(LANE_TO_MOVEMENT_MAPPING).find(
      (key) =>
        LANE_TO_MOVEMENT_MAPPING[key].direction === dir &&
        LANE_TO_MOVEMENT_MAPPING[key].movement === mov
    );
    return (
      junctionData.iot.find((d) => d.iot_id === laneId) || {
        vehicle_count: 0,
        current_signal: "Red",
      }
    );
  ...
  const getLightClass = (data, color) => {
    ...
    if (data.current_signal.toLowerCase() === color) {
      if (color === "red")
        return `${baseClass} bg-[var(--gov-red)] shadow-[0_0_15px_5px_var(--gov-red)]`;
      if (color === "green")
        return `${baseClass} bg-[var(--gov-green)] shadow-[0_0_15px_5px_var(--gov-green)]`;
    }
      ...
      <div className="p-6">
        ...
        <div className="flex justify-center items-center my-6">
          <div className="relative w-[192px] h-[208px]">
            ...
            <div className="absolute bottom-0 left-0 w-full h-20 bg-gray-800 rounded-lg flex justify-center items-center space-x-2 p-2">
              <div
                className={`w-16 h-16 rounded-full text-white flex items-center justify-center ${getLightClass(
                  leftData,
                  "green"
                )}`}
              >
        ...
        <div className="flex justify-between items-center mt-6 text-lg">
          <span className="font-semibold text-[var(--text-secondary)] flex items-center gap-2">
            <span className="material-symbols-outlined">directions_car</span> Vehicle Count:
          </span>
...
// JunctionDetails page, fetching and showing live data
function JunctionDetails() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        console.error("Failed to fetch junction details:", error);
      }
    };
    fetchDetails();
  ...
  if (!junction) {
    return (
      <>
        <Header />
        <h2>Loading Junction Details...</h2>
      </>
    );
  }
      ...
      <main className="flex-1">
        <header className="flex items-center justify-between h-20 px-8 bg-white shadow-md">
          <div className="flex items-center gap-4">
            ...
            <span className="material-symbols-outlined text-4xl text-[var(--gov-green)]">
              traffic
            </span>
          </div>
        </header>
          ...
          <div className="max-w-full mx-auto">
            <div className="mb-10 text-center">
              <h2 className="text-3xl font-bold text-[var(--text-primary)] tracking-tight">
                Junction: {junction.name} (ID: {junction.id})
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
              ...
              <div className="p-4 bg-white rounded-2xl shadow-lg">
                ...
                <p className={`text-2xl font-bold ${getCongestionClass(junction.congestion)} mt-1`}>
                  {junction.congestion}
                </p>
              </div>
              <div className="p-4 bg-white rounded-2xl shadow-lg">
            ...
            <div>
              <h3 className="text-3xl font-bold text-[var(--text-primary)] mb-6 text-center">
                Live Signal Status
              </h3>
        ...
        <div className="fixed bottom-8 right-8">
          <button
            onClick={() => navigate("/")}
            className="bg-[var(--primary-color)] text-white px-6 py-4 rounded-full flex items-center gap-3 hover:bg-green-800 shadow-2xl transform hover:scale-110 transition-transform duration-300"
          >
